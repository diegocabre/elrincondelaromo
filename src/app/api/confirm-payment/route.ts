...
export async function POST(request: Request) {
    try {
        ...
        if (fetchError || !registration) {
            return NextResponse.json({ error: "No se encontró la inscripción en BD" }, { status: 404 });
        }
        ...
        if (updateError) {
            ...
            return NextResponse.json({ error: "No se pudo actualizar el estado de BD" }, { status: 500 });
        }

        // 3. Enviar correo de confirmación con Resend
        if (resend) {
            const workshopTitle = registration.workshops?.title || "Taller Médico/Bienestar";
            const workshopDate = registration.workshops?.date_info || "Fecha por definir";
            ...
            try {
                ...
                await resend.emails.send({
                    from: "El Rincón del Aromo <hola@rincondelaromo.com>",
                    to: userEmail,
                    ...
                    html: `
                        <div style="font-family: Arial, sans-serif; color: #4A3B32; max-width: 600px; margin: 0 auto; border: 1px solid #EACCA4; border-radius: 10px; overflow: hidden;">
                            ...
                            <div style="padding: 30px; background-color: #ffffff;">
                                ...
                                <p style="font-size: 16px; font-weight: bold; color: #8B5E3C; margin-top: 5px;">El Rincón del Aromo</p>
                            </div>
                        </div>
                    `
                });
                ...
                if (adminEmail) {
                    await resend.emails.send({
                        from: "El Rincón del Aromo (Sistema) <hola@rincondelaromo.com>",
                        to: adminEmail,
