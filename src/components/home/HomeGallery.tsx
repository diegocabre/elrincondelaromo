            ...
            {duplicatedImages.map((src, idx) => (
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                    <span className="bg-white/80 p-3 rounded-full text-[#4A3B32] font-semibold text-sm drop-shadow-md backdrop-blur-sm">Ver Foto</span>
                </div>
              </div>
            ))}
          </motion.div>
        ...
        {selectedIndex !== null && (
          ...
          >
            ...
            <div className="absolute bottom-10 left-0 right-0 flex justify-center items-center gap-10 sm:hidden z-50">
                <button onClick={(e) => { e.stopPropagation(); handlePrev(); }} className="text-white/70 hover:text-white p-2 bg-black/40 rounded-full"><ChevronLeft size={32}/></button>
                <span className="text-white/80 text-sm font-medium tracking-widest bg-black/40 px-4 py-1 rounded-full">{selectedIndex + 1} / {images.length}</span>
                <button onClick={(e) => { e.stopPropagation(); handleNext(); }} className="text-white/70 hover:text-white p-2 bg-black/40 rounded-full"><ChevronRight size={32}/></button>
            </div>
